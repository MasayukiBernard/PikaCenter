...
class Inventory:
    def __init__(self, window_status, parent_child_roots_list, db_password):
        ...
        placeholder_val = ". . . Search by Name or Description"
        self.product_search_var.set(placeholder_val)
        ...
        self.product_search_bar.bind('<FocusIn>', partial(self.manage_placeholder, 'in', self.product_search_var, placeholder_val))
        self.product_search_bar.bind('<FocusOut>', partial(self.manage_placeholder, 'out', self.product_search_var, placeholder_val))
        ...
        self.product_tree_btn = {
            ...
            'manage_product_with_entries': ttk.Button(self.product_btn_frame, text="MANAGE PRODUCT w/ ENTRIES", command=partial(self.show_manage_product, 'manage')),
            'manage_sold': ttk.Button(self.product_btn_frame, text="MANAGE SOLD", command=partial(self.show_manage_product, 'manage')),
    ...
    def refresh_product_tree_data(self, *args):
        ...
        if is_search:
            ...
            self.product_search_var.set('. . . Search by Name or Description')
    ...
    def refresh_detail_trees_data(self, event):
        ...
        sql = ["SELECT ", " FROM ", " WHERE refproductkey = :product_key ", " ORDER BY ", " DESC"]
        ...
        order_by = ["eta, entry_date", "sold_date"]
        ...
        for i in range(len(tree_key)):
            ...
            rl_detail = conn.run(sql[0] + column[i] + sql[1] + table[i] + sql[2] + additional_condition[i] + sql[3] + order_by[i] + sql[4], product_key=selected_product_key)
